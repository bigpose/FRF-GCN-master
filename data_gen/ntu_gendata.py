    ...
    with open(file, 'r') as f:
        ...
        for t in range(skeleton_sequence['numFrame']):
            ...
            for m in range(frame_info['numBody']):
                ...
                ]  # 人ID，剪切的边缘，左手置信度，左手状态，右手置信度，右手状态，已重新记录等第三行的十个数据
                body_info = {
                    k: float(v)
...
def gendata(data_path, out_path, ignored_sample_path=None, benchmark='xview', part='eval'):
    ...
    for i, s in enumerate(tqdm(sample_name)):    # i为序号，s为标签名称
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='NTU-RGB-D Data Converter.')  # NTU数据转换器
